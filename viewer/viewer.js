...
function createWorker(self) {
    ...
    function processPlyBuffer(inputBuffer) {
        ...
        // Only process vertex element properties (skip non-vertex elements like extrinsic, intrinsic, etc.)
        const vertexElEnd = header.indexOf("\nelement ", header.indexOf("element vertex") + 1);
        const vertexHdrEnd = vertexElEnd >= 0 ? vertexElEnd : header_end_index;
        for (let prop of header
            .slice(0, vertexHdrEnd)
            .split("\n")
...
async function main() {
    ...
    if (!fileParam) throw new Error("No file specified. Use ?file=path");
    const url = new URL(fileParam, window.location.origin);
    ...
    let splatData = contentLen > 0 ? new Uint8Array(contentLen) : new Uint8Array(1024 * 1024 * 100);
